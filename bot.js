...
const notify = async function(log) {
  ...
  await post(
    "Bot Notified at " + new Date().toISOString(),
    JSON.stringify(data.notifications[0].author)
  );
};
...
const getNotification = async function(cb) {
  await ws
    .call("view.get", [
      "ctzn.network/notifications-count-view",
...
const getNotifications = async function(limit) {
  var res = await ws
    .call("view.get", ["ctzn.network/notifications-view", { limit: limit }])
    .then(function(result) {
      return result;
    })
    .catch(function(error) {
...
const login = async function() {
  var res = await ws
    .call("accounts.login", [
      { username: config.ctzn.user, password: config.ctzn.pass }
    ])
    .then(function(result) {
...
const getPosts = async function(userId,limit) {
  var res = await ws
    .call("view.get", ["ctzn.network/posts-view", userId, { limit: limit, reverse: true }])
    .then(function(result) {
      return result;
    })
    .catch(function(error) {
...
const post = async function(text, body, blob) {
  ...
  if (blob)
    post[2].media = [
      {
        caption: "caption",
        blobs: { original: { blobName: blob, mimeType: "image/png" } }
      }
    ];
  var res = await ws
...
const postCommunity = async function(text, body, blob) {
  ...
  var post = [
    ...
    "ctzn.network/post",
    {
      ...
      extendedTextMimeType: "text/html",
      community: {
        userId: "memes@ctzn.one",
        dbUrl:
          "hyper://0a4968b19d692f472dd39b74b8663d2400dc34e33ca897e3a259c3525ae8168a/"
      }
    }
  ];
  if (blob)
